...
public class JefeChosen extends Composite {
    ...
    public JefeChosen() {
        ...
        jefeChosen = new ValueListBox<>(new Renderer<Empleado>() {
            @Override
        ...
        jefeChosen.addValueChangeHandler(new ValueChangeHandler<Empleado>() {
            @Override
        ...
        comboWrap.add(jefeChosen);
        
        personalREST = new PersonalREST();
        personalREST.addRESTExecutedListener(new BaseREST.RESTExecutedListener() {
            @Override
            public void onRESTExecuted(RESTEvent restEvent) {
                if (restEvent.getDbMethod().equals(ProviderMethod.FIND_JEFES)) {
                    if (restEvent.getDbTypeResult().equals(TypeResult.SUCCESS)) {
                        
                        List<Empleado> jefes = (List<Empleado>) restEvent.getResult();
                        jefeChosen.setAcceptableValues(jefes);
        ...
        personalREST.findJefes();
        
    }
    
    public void setValue(Empleado jefe) {
        jefeChosen.setValue(jefe, true);
    }
    
    public Empleado getValue() {
        return jefeChosen.getValue();
    }
